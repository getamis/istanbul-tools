...
var (
	...
	ErrNoBlock            = errors.New("no block was generated")
...
func (eth *ethereum) ConsensusMonitor(errCh chan<- error, quit chan struct{}) {
	...
	for {
		...
		case <-time.After(10 * time.Second):
			...
			return
		case head := <-subCh:
			// Ensure that new head number is greater than latest number
			if head.Number.Uint64()-1 != latestBlockNumber {
				errCh <- ErrInvalidBlockNumber
			}
